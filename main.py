...
def logo():
 ...
 | |/ / | | |   \  |    | |    | |  \  |%sAuthor by %TR FAHIM%s
 | |    | | | )  | | __ | |____| |   \ |%sFB% Tahsan Rahman Fahim%s
 |_|    |_| |___/  |/  \| |    | |    \|%sTeam %sSECRET CYBER%s
 =======================================%sTools spam otp dengan 29 spammers"""%(qu,pu,ku,qu,pu,ku,qu,pu,ku,qu,qu)
#-------------------------------Input Function------------------------------------------
def input():
  global nom
  nom = raw_input("%s[%s?%s] %sMasukkan nomor target (+880) : "%(pu,me,pu,pu))
  if len(nom) < 5:
    print "%s[%s!%s] %sMasukkan nomor target dengan benar!!"%(pu,me,pu,me)
    input()
  elif nom.startswith(tuple(["88","+880","0"])):
    print "%s[%s!%s] %sMasukkan nomor tanpa 88, +880, ataupun 0\n%s[%s!%s] %sContoh : 01619103775"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
